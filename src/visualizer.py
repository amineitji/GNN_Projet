...
class Visualizer:
    ...
    def plot_main_comparison(self, df_full, best_gat_name):
        ...
        plt.figure(figsize=(10, max(4, len(df_main_plot) * 1.5)))
        g = sns.barplot(
            data=df_main_plot,
            x='q95',
            y='clean_name',
    ...
    def plot_ablation_study(self, df_full, best_gat_name):
        ...
        model_names_to_keep = [best_gat_name, pop_only_name_base, country_only_name_base, pop_only_name_grid,
        ...
        try:
            ...
            best_alpha = best_alpha_series.iloc[0] if not best_alpha_series.empty else -1.0
        except Exception:
            best_alpha = -1.0

        if best_alpha == 1.0:
            df_ablation = df_ablation[~df_ablation['model'].isin([pop_only_name_base, pop_only_name_grid])]
        if best_alpha == 0.0:
            df_ablation = df_ablation[~df_ablation['model'].isin([country_only_name_base, country_only_name_grid])]
    ...
    def plot_grid_search_analysis(self, df_full):
        ...
        g.fig.suptitle('Analyse du Grid Search (Couleur = Score Q95, plus sombre = meilleur)', y=1.05,
                       fontweight='bold')
